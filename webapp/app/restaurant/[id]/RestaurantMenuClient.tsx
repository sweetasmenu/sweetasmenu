...
import { Loader2, Utensils, ShoppingCart, Plus, Minus, X, CheckCircle, MapPin, Clock, Store, Globe, Languages, MessageCircle, Bell, UtensilsCrossed, Droplets, Receipt, HelpCircle, Send, Check, Pencil, ClipboardList, Calendar, Phone, Mail } from 'lucide-react';
import ProtectedImage from '@/components/ProtectedImage';
import { dt, DashboardLanguage, dashboardTranslations } from '@/lib/dashboard-translations';
import { cachedFetch } from '@/lib/utils/cacheManager';

// Dynamic imports - only load the selected template (saves ~40-60KB unused JS)
const ClassicList = dynamic(() => import('./templates/ClassicList'));
const GridView = dynamic(() => import('./templates/GridView'));
const MagazineStyle = dynamic(() => import('./templates/MagazineStyle'));
const Elegant = dynamic(() => import('./templates/Elegant'));
const Casual = dynamic(() => import('./templates/Casual'));
const MobileMenuSection = dynamic(() => import('./templates/MobileMenuSection'));
...
export default function RestaurantMenuClient({ initialData, restaurantId }: { initialData: any; restaurantId: string }) {
  ...
  const [restaurantInfo, setRestaurantInfo] = useState<{ phone?: string; email?: string; address?: string; showEmailOnMenu?: boolean }>({});
  ...
  const [realRestaurantId, setRealRestaurantId] = useState<string>(''); // Real restaurant ID from API

  // Process data from server or client fetch
  const processMenuData = useCallback((data: any) => {
    ...
    if (data.service_options) {
      ...
      if (data.service_options.dine_in) setServiceType('dine_in');
      else if (data.service_options.pickup) setServiceType('pickup');
      else if (data.service_options.delivery) setServiceType('delivery');
    }
    ...
    if (data.delivery_settings) setDeliverySettings(data.delivery_settings);
    if (data.plan) setRestaurantPlan(data.plan);

    if (data.restaurant?.id) {
      setRealRestaurantId(data.restaurant.id);
    }
    if (data.restaurant) {
      setRestaurantInfo({
        ...
        showEmailOnMenu: data.restaurant.show_email_on_menu ?? true,
      });
    }

    // Process consolidated location data
    ...
    // Process consolidated surcharge settings
    if (data.surcharge_settings) {
      setSurchargeSettings({
        credit_card_surcharge_enabled: data.surcharge_settings.credit_card_surcharge_enabled || false,
        credit_card_surcharge_rate: data.surcharge_settings.credit_card_surcharge_rate || 2.50
      });
    }

    // Process consolidated food surcharge settings
    if (data.food_surcharge_settings) {
      setFoodSurchargeSettings({
        food_surcharge_enabled: data.food_surcharge_settings.food_surcharge_enabled || false,
        food_surcharge_rate: data.food_surcharge_settings.food_surcharge_rate || 10.00,
        food_surcharge_name: data.food_surcharge_settings.food_surcharge_name || 'Holiday Surcharge'
      });
    }

    // Process consolidated bestsellers
    const bestSellersArr = data.best_sellers || [];
    if (bestSellersArr.length > 0) {
      const bestSellerIds = new Set(bestSellersArr.map((item: any) => item.menu_id));
      ...
      const bestSellerItems: MenuItem[] = bestSellersArr.map((item: any) => ({
  ...
  // Client-side fetch fallback (used when no initialData from server)
  const fetchMenus = async () => {
    try {
      const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      const data = await cachedFetch<any>(
        `${API_URL}/api/public/menu/${restaurant_id}`,
        {},
        5 * 60 * 1000 // 5-minute client-side cache
      );
  ...
  const groupedMenus = useMemo(() => {
    ...
    return groups;
  }, [menus, bestSellers, branding.category_order, originalMenus]);
      ...
      <div className="relative overflow-hidden">
        ...
        >
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10">
            ...
            {/* Restaurant Info & Operating Hours */}
            <div className="mt-3 space-y-1.5 text-white/90 text-xs sm:text-sm">
              {/* Contact Info */}
              {(restaurantInfo.phone || (restaurantInfo.email && restaurantInfo.showEmailOnMenu) || restaurantInfo.address) && (
                <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
                  {restaurantInfo.phone && (
                    <a href={`tel:${restaurantInfo.phone}`} className="inline-flex items-center gap-1 hover:text-white transition-colors">
                      <Phone className="w-3.5 h-3.5" />
                      <span>{restaurantInfo.phone}</span>
                    </a>
                  )}
                  {restaurantInfo.email && restaurantInfo.showEmailOnMenu && (
                    <a href={`mailto:${restaurantInfo.email}`} className="inline-flex items-center gap-1 hover:text-white transition-colors">
                  ...
                  <div className="flex flex-col items-center gap-0.5">
                    {groups.map((g, i) => (
                      <div key={i} className="inline-flex items-center gap-1.5">
                        {i === 0 && <Clock className="w-3.5 h-3.5 flex-shrink-0" />}
                        {i > 0 && <span className="w-3.5" />}
                        <span>{formatDays(g.days)} {formatTime(g.open)}-{formatTime(g.close)}</span>
                      </div>
                    ))}
                    {closedDays.length > 0 && (
                      <div className="inline-flex items-center gap-1.5 text-white/60">
                        {groups.length === 0 && <Clock className="w-3.5 h-3.5 flex-shrink-0" />}
                        {groups.length > 0 && <span className="w-3.5" />}
                        <span>{closedDays.join(', ')} Closed</span>
                      </div>
                    )}
                  </div>
                );
...
function EditCartItemModal({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[55] md:flex md:items-center md:justify-center md:p-4" onClick={onClose}>
      <div
        className="bg-white md:rounded-2xl w-full md:max-w-lg overflow-y-auto mobile-modal-height md:relative md:h-auto md:max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
